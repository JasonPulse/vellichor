...
/// <summary>
/// In-place deobfuscation of MMB (0x2e) and MZB (0x1c) chunk payloads. FFXI scrambles
/// zone geometry on disk; the payload must be decoded before the struct layout applies.
/// Ported verbatim from galkareeve `decode_mmb` / `decode_mmb2` / `decode_mzb`
/// (DatLoader/TDWAnalysis.cpp), including the two 256-byte key tables.
/// </summary>
public static class DatCrypt
{
    /// <summary>Decode an MMB payload in place. maxLen = payload length. Returns false if malformed.</summary>
    public static bool DecodeMmb(byte[] p, int maxLen)
    {
        if (p.Length >= 4 && p[3] >= 5)
        {
            int decodeLength = p[0] | (p[1] << 8) | (p[2] << 16);
            if (decodeLength > maxLen) return false;

            uint key = KeyTable[p[5] ^ 0xF0];
            int keyCounter = 0;
            for (int pos = 8; pos < decodeLength; pos++)
            {
                uint x = ((key & 0xFF) << 8) | (key & 0xFF);
                key += (uint)(++keyCounter);
                p[pos] ^= (byte)(x >> (int)(key & 7));
                key += (uint)(++keyCounter);
            }
        }
        DecodeMmb2(p);
    ...
    private static void DecodeMmb2(byte[] p)
    {
        if (p.Length < 8 || p[6] != 0xFF || p[7] != 0xFF) return;
        int decodeLength = p[0] | (p[1] << 8) | (p[2] << 16);
        uint key1 = (uint)(p[5] ^ 0xF0);
        uint key2 = KeyTable2[key1];

        uint decodeCount = (uint)(((decodeLength - 8) & ~0xf) / 2);
        int a = 8;                        // data1 (dword pair) base
        int b = 8 + (int)decodeCount;     // data2 base
        for (uint pos = 0; pos < decodeCount; pos += 8)
        {
            if ((key2 & 1) != 0 && b + 8 <= p.Length && a + 8 <= b)
            {
                for (int i = 0; i < 8; i++) (p[a + i], p[b + i]) = (p[b + i], p[a + i]);
            }
            key1 += 9;
            key2 += key1;
            a += 8;
    ...
    /// <summary>Decode an MZB payload in place. maxLen = payload length. Returns false if malformed.</summary>
    public static bool DecodeMzb(byte[] p, int maxLen)
    {
        if (p.Length < 8 || p[3] < 0x1B) return true; // not obfuscated for this variant
        int decodeLength = p[0] | (p[1] << 8) | (p[2] << 16);
        if (decodeLength > maxLen) return false;

        uint key = KeyTable[p[7] ^ 0xFF];
        int keyCounter = 0;
        for (uint pos = 8; pos < decodeLength;)
        {
            uint xorLength = ((key >> 4) & 7) + 16;
            if ((key & 1) != 0 && pos + xorLength < decodeLength)
                for (uint i = 0; i < xorLength; i++) p[pos + i] ^= 0xFF;
            key += (uint)(++keyCounter);
            pos += xorLength;
        }

        int nodeCount = p[4] | (p[5] << 8) | (p[6] << 16);
        int node = 32;                    // SMZBBlock100 array start
        for (int i = 0; i < nodeCount && node + 16 <= p.Length; i++, node += 100)
            for (int j = 0; j < 16; j++) p[node + j] ^= 0x55;
        return true;
    }
